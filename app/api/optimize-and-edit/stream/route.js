...
原始提示词（可能是中文）：
${rawPrompt}`
      : `你是一名资深图像提示词工程师。现在是图生图（image edit）场景。用户未提供文字提示词，请仅基于上传的参考图片，自动提取并生成面向 AI 图像编辑的高质量 ${outputLang} Prompt，要求：
- 准确提炼图片的主体、环境、构图、镜头、光照、材质、配色、风格等关键视觉信息；
- 强调在保持参考图主体与风格的前提下，适度提出可控的细节/风格/光效增强建议；
- 使用逗号分隔短语，避免长句与分段；
- 输出为单段；中文不超过100字；英文不超过50词；
- 不要包含画幅比例、尺寸、参数等技术指标（如 3:2, 16:9, 1024x1024 等）；
